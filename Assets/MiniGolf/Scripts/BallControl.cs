...
public class BallControl : MonoBehaviour
{
    ...
    if (canShoot)
    {
        ...
        float currentSpeed = rgBody.velocity.magnitude;
        float adjustedForce = force;
        if (currentSpeed > MaxVelocity)
        {
            adjustedForce = Mathf.Lerp(force, 0, (currentSpeed - MaxVelocity) / MaxVelocity);
        }

        rgBody.AddForce(direction.normalized * adjustedForce, ForceMode.Impulse);
    }
    }   
